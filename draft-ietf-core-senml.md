...
controlleroller has set the time of the lights coming on to 20 ms in the future
...
network that has a high odds of delivering a message is less than 100 ms and
...
dimmed to 50% brightens immediately to give the user instant feedback that
...
perception of both lights going off at the same time vs the annoying popcorn
effect.
...
* For streaming SensML in CBOR representation, the array containing the
  records SHOULD be an CBOR indefinite length array while for non
